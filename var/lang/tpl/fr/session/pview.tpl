			...
			$.post(
				...
				function(data, textStatus, jqXHR) {
					if(textStatus == 'success') {
						var message = data ?
							'Mise à jour réussie de "'+data["name"]+'" à "'+data["value"]+'".' :
							'Une erreur s&eacute;est produite en mettant à jour les permissions';
						%{msg message}%
					}
					else if(textStatus == 'error')
						%{msg "Une erreur s&eacute;est produite en mettant à jour les permissions", true}%
					else if(textStatus == 'timeout')
						%{msg "La connexion a expirée", true}%
				},
				"json"
			);
			return false;
